...
class product_service:
    ...
    @staticmethod
    def update_stock(prod_id: int, new_stock: int):
        # Get the product
        product = product_dao.get_product_by_id(prod_id)
        if not product:
            raise Exception(f"Product with id {prod_id} not found")
        
        # Update stock
        updated = product_dao.update_product(prod_id, {"stock": new_stock})
        if not updated:
            raise Exception("Failed to update product stock")

        # Return updated product info
        return product_dao.get_product_by_id(prod_id)
